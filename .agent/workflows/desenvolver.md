...
- NUNCA simplificar um efeito porque "é complexo" (Ex: Se pedir particles.js, rode particles.js puro e use lazy loading)
...
- NUNCA usar `opacity: 0` ou `transform` (ou data-aos) em elementos do Hero (O Hero deve ter zero dependência de interatividade para aparecer no First Paint e CLS=0).
- NUNCA gerar imagens usando IA (são quadradas e inadequadas). Use fundos CSS ou URLs da source referenciadas em Unsplash.
...
- [ ] Hero SEM NENHUMA animação de entrada estrutural que o esconda no carregamento (sem opacity:0, fade-in, data-aos).
